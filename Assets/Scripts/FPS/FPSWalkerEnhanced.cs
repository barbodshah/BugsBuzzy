...
public class FPSWalkerEnhanced : MonoBehaviour
{
	...
	[HideInInspector] public Vector3 m_MoveDirection = Vector3.zero;
	...
	private void Start()
	{
		...
		//ignore collision between player and dead bodies
	...
	private void Update()
	{
		...
		running = m_Speed == m_RunSpeed;
	}
	private void FixedUpdate()
	{
		float inputX = FPSInputManager.Instance.horizontal;
		float inputY = FPSInputManager.Instance.vertical;
		...
		if (m_Grounded)
		{
			...
			if ((m_sliding && m_SlideWhenOverSlopeLimit) || (m_SlideOnTaggedObjects && m_Hit.collider.tag == "Slide"))
			{
